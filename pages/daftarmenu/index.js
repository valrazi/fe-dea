...
function daftarmenu() {
  ...
  const [eMenuId, setEditMenuId] = useState()
  const [eNamaMenu, setEditNamaMenu] = useState('');
  const [eLinkMenu, setEditLinkMenu] = useState('');
  const [eIconMenu, setEditIconMenu] = useState('');
  const [eUrutanMenu, setEditUrutanMenu] = useState(0);
  const [eIsOn, setEditIsOn] = useState('');

  const [eDropdown, setEditDropdown] = useState(null)
  async function editMenu(menu) {
    setEditVisible(true)
    setEditMenuId(menu.id)
    setEditNamaMenu(menu.nama)
    setEditLinkMenu(menu.link)
    setEditIconMenu(menu.icon)
    setEditUrutanMenu(menu.order)
    setEditIsOn(menu.isOn)
    if(menu.isOn == 0) {
      setEditDropdown(isOnList[0])
    }else {
      setEditDropdown(isOnList[1])
    }
  }

  function editMenuAPI(menuId) {
    axios.put(`${process.env.NEXT_PUBLIC_BACKENDURL}/admin/menu/${menuId}`, {
      nama: eNamaMenu,
      link: eLinkMenu,
      icon: eIconMenu,
      order: eUrutanMenu,
      ...
      alert(`Menu with ID ${eDropdown.code} succesfully updated!`)
      console.log(res);
              ...
              <div className="col-12">
                <div className="card">
                      ...
                      <div className="field grid">
                        ...
                        <div className="col-12 md:col-10">
                          <InputText id="email3" type="text"
                            value={eNamaMenu} onInput={(e) => setEditNamaMenu(e.target.value)} />
                        </div>
                      </div>
                      <div className="field grid">
                        <label htmlFor="email3" className="col-12 mb-2 md:col-2 md:mb-0">
                        ...
                        <div className="col-12 md:col-10">
                          <InputText id="email3" type="text"
                            value={eLinkMenu} onInput={(e) => setEditLinkMenu(e.target.value)} />
                        </div>
                      </div>
                      <div className="field grid">
                        <label htmlFor="email3" className="col-12 mb-2 md:col-2 md:mb-0">
                        ...
                        <div className="col-12 md:col-10">
                          <InputText id="email3" type="text"
                            value={eIconMenu} onInput={(e) => setEditIconMenu(e.target.value)} />
                        </div>
                      </div>
                      <div className="field grid">
                        <label htmlFor="email3" className="col-12 mb-2 md:col-2 md:mb-0">
                        ...
                        <div className="col-12 md:col-10">
                          <InputNumber id="email3" type="text"
                            value={eUrutanMenu} onValueChange={(e) => setEditUrutanMenu(e.value)} />
                        </div>
                      </div>
                      <div className="field grid">
                        <label htmlFor="email3" className="col-12 mb-2 md:col-2 md:mb-0">
                        ...
                        <div className="col-12 md:col-10">
                        <Dropdown value={eDropdown} onChange={(e) => setEditDropdown(e.value)} options={isOnList} optionLabel="name"
                  ...
                  <div className="col-4 md:col-4">
                    <Button label="Ubah" severity="warning" style={{ marginRight: '10px' }}
                      onClick={() => editMenuAPI(eMenuId)}></Button>
                  </div>
                </div>
              </div>
            </Dialog>
